...
LEFT JOIN notificacao ON notificacao.age_id = disputa_reembolso.age_id
ORDER BY dsp_abertura DESC, his_data DESC;
...
FROM prestador
JOIN agendamento_servico ON agendamento_servico.pre_id = prestador.pre_id
JOIN pagamento ON pagamento.age_id = agendamento_servico.age_id
WHERE pagamento.pag_status = 'Pago'
...
SELECT
  ...
  COUNT(pagamento.pag_id)                 AS 'Total de pagamentos',
  SUM(pagamento.pag_valor)                AS 'Receita total paga (R$)',
  AVG(pagamento.pag_valor)                AS 'Ticket médio (R$)',
  MIN(pagamento.pag_data)                 AS 'Primeira data',
  MAX(pagamento.pag_data)                 AS 'Última data'
FROM pagamento
WHERE pagamento.pag_status = 'Pago'
GROUP BY DATE_FORMAT(pagamento.pag_data, '%Y-%m'),
